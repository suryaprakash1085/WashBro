...
export default function AdminHomepage() {
  ...
  // Database homepage content state
  ...
  const fetchHomepageContent = async (start?: string | null, end?: string | null) => {
    setIsLoading(true);
    try {
      ...
      const data = await apiFetch(`/api/homepage?${params.toString()}`, 'GET');
      setDbContent(data.data || []);
    } catch (error) {
      toast({ title: 'Error', description: 'Failed to load homepage content' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDateRangeChange = (start: string | null, end: string | null) => {
    setStartDate(start);
    setEndDate(end);
    fetchHomepageContent(start, end);
  };

  const handleAddContent = async () => {
    if (!newContent.title || !newContent.section) {
      toast({ title: 'Error', description: 'Title and section are required' });
      return;
    }

    try {
      await apiFetch('/api/homepage', 'POST', newContent);
      setNewContent({ title: '', section: '', description: '', content: '' });
      setShowAddModal(false);
      toast({ title: 'Success', description: 'Content added successfully' });
      fetchHomepageContent(startDate, endDate);
    } catch (error) {
  ...
  const handleDeleteContent = async (id: number) => {
    ...
    try {
      await apiFetch(`/api/homepage/${id}`, 'DELETE');
